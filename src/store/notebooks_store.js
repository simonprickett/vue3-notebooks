...
import {request_crate, request_python} from "@/store/http/requests";
...
export const use_notebooks_store = defineStore('notebooks', () => {
    ...
    async function run_cell(cell_index, cell_id) {
      ...
      if (cell.language === 'python'){
        response = await request_python(cell.content)
      } else {
        response = await request_crate(
          cell.content,
          'error_trace=true',
          {},
          true)
      ...
      if (response.ok) {
        if (cell.language === 'python') {
          ...
          cell.response.label = json_response.stdout
          cell.response.data = {
            rows: [],
            headers: [],
            row_count: 0
